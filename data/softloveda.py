...
class SoftLoveDA(Dataset):
    ...
    def batch_generate(self, image_dir, mask_dir):
        ...
        if mask_dir is not None:
            for fname in rgb_filename_list:
                cls_filepath_list.append(os.path.join(mask_dir, fname.replace(".png",".npy")))
    ...
    def __getitem__(self, idx):
        ...
        if len(self.cls_filepath_list) > 0:
             # 3378 3276
            mask = np.load(self.cls_filepath_list[idx]).transpose(1,2,0)
...
class SoftLoveDALoader(DataLoader, ConfigurableMixin):
    def __init__(self, config):
        ConfigurableMixin.__init__(self, config)
        dataset = SoftLoveDA(self.config.image_dir, self.config.mask_dir, self.config.transforms)
        ...
        super(SoftLoveDALoader, self).__init__(dataset,
